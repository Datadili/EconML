...
# Iterate over www_columns
for (column in www_columns) {
  # Construct the full model formula
  model <- paste(column, regressors)
  
  # Fit linear regression model and compute residuals
  df <- df %>%
    mutate(!!column := residuals(lm(as.formula(model), data = .))) 
}
