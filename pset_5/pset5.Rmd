...
library(knitr) #has kable, used for latex
library(kableExtra)
...
```
\textit{iii}
Note: Could not find lambdas with 1 or 2, regressors. Finding 3, 4, 5 regressors for the remaining questions... 
```{r}
#making a function to find minimum lambda that has a certain number of regressors
min_lamba_regressors <- function(num_of_regressors) {
  ...
  for (lasso_value in lasso_mod$lambda){
    ...
    #if the number of coeffiecients in a lambda value is ---, append
...
lasso_3regs <- min_lamba_regressors(3)
...
\textit{iv}
```{r}
lasso_4regs <- min_lamba_regressors(4)
...
\textit{v}
```{r}
lasso_5regs <- min_lamba_regressors(5)
#get the coefficients (no intercept)
...
**17.i**
This is a low-dimensional dataset because the number of predictors is smaller than the number of observations.
